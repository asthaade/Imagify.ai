...
const Header = () => {
    ...
    return (
        ...
        >
            ...
            >
                Empower your creativity with AI. Transform ideas into stunning visuals in
                seconds. Just type and see the results.
            </motion.p>

            <motion.button onClick={onClickHandler} className="sm:text-lg text-white bg-black w-auto mt-8 mx-12 px-8 py-3 flex items-center gap-2 rounded-full"
            ...
            <motion.div className="flex flex-wrap justify-center mt-14 gap-3"
            initial={{opacity:0}}
            animate={{opacity:1}}
            transition={{deplay:1, duration:1}}
            >
                {Array(6).fill().map((item,index)=>(
                    <motion.img
                    whileHover={{scale:1.05, duration:0.1}}
                    className="rounded hover:scale-105 transaction-all
                    duration-300 cursor-pointer max-sm:w-10"
                    src={index%2 === 0 ? assets.sample_img_3 :
                    assets.sample_img_5} alt="" key={index} width={70}/>
                ) )}
            </motion.div>
            <motion.p
            initial={{opacity:0}}
